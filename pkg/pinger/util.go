...
func (e *Exporter) getOvsStatus() (bool, error) {
	...
	for _, component := range components {
		...
		log.Debugf("%s: getOvsStatus() calls GetProcessInfo(%s)", e.Client.System.ID, component)
		if err != nil {
		...
		log.Debugf("%s: getOvsStatus() completed GetProcessInfo(%s)", e.Client.System.ID, component)
	}
...
func (e *Exporter) ovsDatapathLookupsMetrics(line, datapath string) {
	...
	for _, field := range strings.Fields(s) {
		...
		if err != nil {
			log.Errorf("Failed to parse value %v into float in DpFlowsLookup:(%v)", value, err)
			value = 0
		}
		switch elem[0] {
		case "hit":
...
func (e *Exporter) ovsDatapathMasksMetrics(line, datapath string) {
	...
	for _, field := range strings.Fields(s) {
		...
		value, err := strconv.ParseFloat(elem[1], 64)
		if err != nil {
			log.Errorf("Failed to parse value %v into float in OvsDpMasks:(%v)", value, err)
			value = 0
		}
		switch elem[0] {
		case "hit":
...
func (e *Exporter) getInterfaceInfo() ([]*ovsdb.OvsInterface, error) {
	...
	if intfs, err = e.Client.GetDbInterfaces(); err != nil {
		log.Errorf("GetDbInterfaces error: %v", err)
		e.IncrementErrorCounter()
		return nil, err
	}
