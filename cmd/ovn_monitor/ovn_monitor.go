...
import (
	...
	ovn "github.com/alauda/kube-ovn/pkg/ovn_monitor"
...
func main() {
	...
	flag.StringVar(&listenAddress, "listen-address", ":9999", "Address to listen on for web interface and telemetry.")
	flag.StringVar(&metricsPath, "telemetry-path", "/metrics", "Path under which to expose metrics.")
	flag.IntVar(&pollTimeout, "ovs.timeout", 2, "Timeout on JSON-RPC requests to OVN.")
	flag.IntVar(&pollInterval, "ovs.poll-interval", 15, "The minimum interval (in seconds) between collections from OVN server.")
	...
	flag.StringVar(&databaseNorthboundFileDataPath, "database.northbound.file.data.path", "/etc/ovn/ovnnb_db.db", "OVN NB db file.")
	...
	flag.StringVar(&databaseSouthboundFileDataPath, "database.southbound.file.data.path", "/etc/ovn/ovnsb_db.db", "OVN SB db file.")
	...
	flag.StringVar(&serviceNorthdFilePidPath, "service.ovn.northd.file.pid.path", "/var/run/ovn/ovn-northd.pid", "OVN northd daemon process id file.")
	...
	err := exporter.StartConnection()
	if err != nil {
		log.Errorf("%s failed to connect db socket properly: %s", ovn.GetExporterName(), err)
